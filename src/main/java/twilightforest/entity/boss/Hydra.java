...
public class Hydra extends Mob implements Enemy, EnforcedHomePoint {
	...
	public boolean attackEntityFromPart(HydraPart part, DamageSource source, float damage) {
		...
		if (!this.level().isClientSide() && source == DamageSource.IN_WALL) {
	...
	public void die(DamageSource cause) {
		...
		if (!this.level().isClientSide()) {
			TFGenerationSettings.markStructureConquered(this.level(), new BlockPos(this.blockPosition()), TFLandmark.HYDRA_LAIR);
